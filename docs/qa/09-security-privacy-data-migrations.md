...
| `SEC-023` | Browser localStorage caches | Local-only persistence under Helmryth-owned keys | Browser storage contains older Helmryth webhook cache documents or malformed JSON | Seed the exact fixtures defined by `src/lib/webhook-credentials.test.ts`, plus malformed connected-capability cache values; load Helmryth UI | Older cache documents are rewritten to the current shape without secrets; malformed records are dropped without crashing; no credentials enter the connected-capability cache | Confirm every write uses a Helmryth-owned key | Re-run `src/lib/webhook-credentials.test.ts`, `src/lib/analytics.test.ts`, and `src/lib/connected-apps-cache.test.ts` | Clear browser storage | `P1` |
| `SEC-024` | Product telemetry | Explicit opt-in and HTTPS-only sink | Build contains telemetry runtime values | Start with no consent, an explicit opt-out, invalid HTTP host, and valid HTTPS host after explicit opt-in | Telemetry stays off by default; an explicit opt-out remains off; invalid non-HTTPS host disables telemetry; `identifyEmail` remains no-op | Confirm autocapture and pageview remain disabled and no personal identifier is sent | Automated and re-run: `src/lib/analytics.test.ts` | Clear localStorage and reset test globals | `P0` |
